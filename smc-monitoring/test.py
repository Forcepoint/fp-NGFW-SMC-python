...
from smc_monitoring.wsocket import SMCSocketProtocol, SessionNotFound, websocket, websocket_debug
...
def get_field_schema_by_name(fields, max_ids=2000):
    # List of fields, expecting the value to match 'pretty' (what is shown in
    # SMC UI)
...
if __name__ == "__main__":
    ...
    pprint(vars(query))
    for batch in query.fetch_batch(
            CSVFormat, max_recv=1):  # is max_recv mandatory?
        print(batch)
    ...
    # "fetch":{}, "format":{"type":"texts", "field_format": "name"}}
    ...
    # query.request = {"query":{"definition":"BLACKLIST","target":"lynn"},
    # "fetch":{}, "format":{"type":"texts"}}

    # myfilter = InFilter(FieldValue(LogField.SRC), [IPValue('192.168.4.82'),
    # IPValue('172.18.1.152')])

    # Use case to pull specific filter logs from Audit
    #     query = LogQuery(backwards=False)
    #     query.format.timezone('CET')
    #     default_audit_fields_ids = [LogField.TIMESTAMP, LogField.DATATYPE,
    #     LogField.USERORIGINATOR, LogField.TYPEDESCRIPTION, LogField.RESULT,
    #     LogField.OBJECTNAME, LogField.OBJECTID, LogField.OBJECTTYPE, LogField.INFOMSG]
    #     query.format.field_ids(default_audit_fields_ids)
    #     # Show only Audit log entries
    #     query.add_in_filter( FieldValue(LogField.DATATYPE),
    #     [ConstantValue(DataType.AUDIT)]) #HINT: it could be nice to have
    #     the Audit Data Type as constant in the LogField
    #
    ...
    # If the engine is not initialized, the query sits in select blocking.
    # Should check for a connected engine first?
    ...
    # query.add_in_filter(FieldValue(LogField.NODEID),
    # [ElementValue(Host('Sidewinder v8'))]) #<-- Doesnt work!
