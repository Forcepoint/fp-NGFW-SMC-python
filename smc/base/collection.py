...
class SubElementCollection(object):
    ...
    def _load_from_engine(self, engine, reference):
        ...
        for r in engine.data.get(reference, []):
            for _, data in r.items():
                ...
                res = self.cls(name=cache.get("name"), href=cache.get_link("self"), type=cache.type)
                res.data = cache
                res._engine = engine
    ...
    def _fetch_all(self):
        if self._result_cache is None:
            results = (
                smc.base.model.prepared_request(FetchElementFailed, href=self.href).read().json
            )
...
def sub_collection(href, cls):
    ...
    """
    return type(cls.__name__, (CreateCollection,), {})(href, cls)
...
def create_collection(href, cls):
    ...
    return type(cls.__name__, (SubElementCollection,), {"create": meth})(href, cls)
...
class ElementCollection(object):
    ...
    def _list(self):
        try:
            ...
            _list = (
                smc.base.model.prepared_request(
                    FetchElementFailed,
    ...
    def __repr__(self):
        href = self._params.get('href')
        if href is not None:
            query = ['{}={}'.format(q, v) for q, v in self._params.items() if q != "href"]
            return '{}(GET {}?{})'.format(self.__class__.__name__, href,
                                          '&'.join(query) if query else '')
        else:
            query = ["{}={}".format(q, v) for q, v in self._params.items()]
            return "{}(GET /elements?{})".format(self.__class__.__name__,
                                                 "&".join(query) if query else "")
    ...
    def filter(self, *filter, **kw):  # @ReservedAssignment
        ...
        if not exact_match and self._params.get("filter_context", {}) in (
            "network",
            "address_range",
            "network_elements",
        ):
        ...
        return self._clone(
            filter=_filter, iexact=iexact, exact_match=exact_match, case_sensitive=case_sensitive
        )
...
class CollectionManager(object):
    ...
    def iterator(self, **kwargs):
        ...
        # have to check if element type is a composite type
        # @see API FilterElementContext (@see engine_clusters)
        # in this case it is needed to search using filter_context query param
        filter_context_types = ["engine_clusters", "network_elements", "alias", "fw_clusters",
                                "ips_clusters", "layer2_clusters", "services",
                                "services_and_applications", "tags", "situations"]
        if self._cls.typeof in filter_context_types:
            params = {"filter_context": self._cls.typeof}
        else:
            params = {'href': self._cls.href}
        params.update(kwargs)
    ...
    def filter(self, *filter, **kw):  # @ReservedAssignment
        ...
        if (
            not exact_match
            and hasattr(self, "_cls")
            and self._cls.typeof in ("network", "address_range")
        ):
            _filter = _strip_metachars(_filter)
...
class Search(ElementCollection):
    ...
    def context_filter(self, context):
        ...
        raise InvalidSearchFilter(
            "Context filter %r was invalid. Available filters: %s" % (context, CONTEXTS)
        )
    ...
    def unused(self):
        ...
        :rtype: list(Element)
        """
        self._params.update(href=self._resource.get("search_unused"))
        return self
    ...
    def duplicates(self):
        ...
        :rtype: list(Element)
        """
        self._params.update(href=self._resource.get("search_duplicate"))
        return self
