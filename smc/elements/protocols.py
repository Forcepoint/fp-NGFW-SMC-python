...
.. note:: The `description` field of the parameter matches what you would see in the SMC under
...
class ProtocolParameterValue(object):
    ...
    :ivar str description: The read-only description of this setting, used in SMC
            ...
            if not proxy_server and self._inspected_service_ref:
                ...
                if self.protocol_agent.name not in enabled_services:
                    raise MissingDependency(
                        "The specified ProxyServer %r does not enable "
    ...
    def _update(self, **kwargs):
        if "tls_policy" in kwargs:
            ...
            if not tls_policy and self._tls_inspection_policy_ref:
                ...
                self.protocol_agent_values.update(tls_inspection_policy_ref=tls_policy.href)
                return True
        return False
...
class ProtocolAgentValues(BaseIterable):
    ...
    the value you would see within the SMC for the given field.
    ...
    _class_map = {
        "proxy_service_value": ProxyServiceValue,
    ...
    def __init__(self, protocol_agent, values):
        ...
        for parameter_value in values:
            for value_type, val in parameter_value.items():
                items.append(
                    type(
                        str(value_type.title().replace("_", "")),
                        (self._class_map.get(value_type, ProtocolParameterValue),),
                        {"protocol_agent": protocol_agent, "protocol_agent_values": val},
                    )()
                )
