...
def get_container_stats(container_id, stream=False):
    ...
    """
    response = do_get("containers/{}/stats".format(container_id), params={"stream": stream})
    return json.loads(response.text)
...
def exec_create(container_id):
    ...
    cmd = {
        ...
        "Cmd": [
            "/bin/sh",
            "-c",
            "/etc/init.d/sgMgtServer start && " "/etc/init.d/sgLogServer start",
        ],
    }
...
def get_available_smc_versions():
    ...
    """
    return [repotag for image in get_images(filter="d70/smc") for repotag in image.get("RepoTags")]
...
def smc_version_simple():
    ...
    :rtype: list
    """
    return [tag.split(":")[-1] for tag in get_available_smc_versions()]
...
if __name__ == "__main__":
    ...
    if len(sys.argv[1:]) > 0:
        ...
        if argument.startswith("version"):
            ...
            if argument not in smc_version_simple():
                ...
                sys.exit(1)
            smc["Image"] = "d70/smc:{}".format(argument)
    ...
    for i in range(3):
        try:
            ...
            if e.message.get("message").startswith(
