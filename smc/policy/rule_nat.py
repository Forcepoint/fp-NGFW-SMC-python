...
class NATValue(NestedDict):
    ...
    def _update_field(self, natvalue):
        ...
        if natvalue.element and natvalue.element != self.element:
            ...
            natvalue.ip_descriptor
            and self.ip_descriptor
            and natvalue.ip_descriptor != self.ip_descriptor
        ):
...
class NATElement(NestedDict):
    ...
    def automatic_proxy(self):
        ...
        :rtype: bool
        """
        return self.get(self.typeof, {}).get("automatic_proxy")

    @automatic_proxy.setter
    def automatic_proxy(self, value):
        self.setdefault(self.typeof, {}).update(automatic_proxy=value)
    ...
    def update_field(self, element_or_ip_address=None, start_port=None, end_port=None, **kw):
        ...
        if not self.translated_value:
            ...
            if "dynamic_src_nat" in self.typeof:
                src.update(min_port=start_port or 1024, max_port=end_port or 65535)
        ...
        if automatic_proxy is not None and self.automatic_proxy != automatic_proxy:
    ...
    def translated_value(self):
        ...
        if self.typeof in self:
            return NATValue(self.get(self.typeof, {}).get("translated_value"))
...
class StaticDestNAT(NATElement):
    ...
    def update_field(
        self, element_or_ip_address=None, original_port=None, translated_port=None, **kw
    ):
        """
        ...
        if not self.translated_value:
            ...
            if original_port:
                if self.original_value._update_field(NATValue(_extract_ports(original_port))):
                    updated = True
    ...
    def original_value(self):
        if self.typeof in self:
            return NATValue(self.get(self.typeof, {}).get("original_value"))
...
class DynamicSourceNAT(NATElement):
    ...
    def translated_value(self):
        ...
        """
        if self.typeof in self:
            return NATValue(self.get(self.typeof, {}).get("translation_values")[0])
