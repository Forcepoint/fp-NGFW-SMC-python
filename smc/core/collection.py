...
class TunnelInterfaceCollection(InterfaceCollection):
    ...
    def add_cluster_virtual_interface(self, interface_id, cluster_virtual=None,
                    network_value=None, nodes=None, zone_ref=None, comment=None, **kw):
        """
...
class PhysicalInterfaceCollection(InterfaceCollection):
    ...
    def _add_65(self, interface_id, virtual_mapping=None,
    ...
    def _add_66(self, interface_id, virtual_mapping=None,
            virtual_resource_name=None, zone_ref=None, comment=None):
        if virtual_mapping is not None:
            virtual_resource_settings = [{'virtual_resource_name': virtual_resource_name,
                'virtual_mapping': virtual_mapping}]
        else:
            virtual_resource_settings = None

        interface = Layer3PhysicalInterface(engine=self._engine,
            interface_id=interface_id, zone_ref=zone_ref, comment=comment,
    ...
    def add_layer3_vlan_interface(self, *args, **kwargs):
        ...
        :return: None
        """
        versioned_method = get_best_version(('6.5', self._add_layer3_vlan_interface_65),
                                                ('6.6', self._add_layer3_vlan_interface_66))
        return versioned_method(*args, **kwargs)

    def _add_layer3_vlan_interface_65(self, interface_id, vlan_id, address=None,
    ...
    def _add_layer3_vlan_interface_66(self, interface_id, vlan_id, address=None,
        network_value=None, virtual_mapping=None, virtual_resource_name=None,
        zone_ref=None, comment=None, **kw):

        if virtual_mapping is not None:
            virtual_resource_settings = [{'virtual_mapping': virtual_mapping,
                'virtual_resource_name': virtual_resource_name}]
        else:
            virtual_resource_settings = []
        interfaces = {'nodes': [{'address': address, 'network_value': network_value}] if address
            and network_value else [], 'zone_ref': zone_ref,
            'virtual_resource_settings': virtual_resource_settings, 'comment': comment}
    ...
    def add_inline_interface(self, interface_id, second_interface_id,
        ...
        interface_spec = {'interface_id': interface_id, 'second_interface_id': second_interface_id,
            'interface': interface if self._engine.type in ('single_fw', 'fw_cluster')
