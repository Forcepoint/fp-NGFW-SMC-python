...
first getting the top level interface, and calling
    :func:`~smc.core.interfaces.Interface.vlan_interface`
...
class InterfaceOptions(object):
    ...
    * The default IP address for outgoing traffic
    ...
    def set_primary_mgt(self, interface_id, auth_request=None, address=None):
        ...
        contact. For single engine and cluster engines, this will enable 'Outgoing',
        ...
        For virtual engines, only auth_request and outgoing will be set.
...
class QoS(object):
    ...
    def disable(self):
        ...
        """
        self._interface.data.update(qos_limit=-1, qos_mode="no_qos", qos_policy_ref=None)
    ...
    def qos_policy(self):
        ...
        :rtype: QoSPolicy
        """
        return QoSPolicy.from_href(self._interface.data.get("qos_policy_ref", None))
    ...
    def statistics_only(self):
        ...
        tagged by QoS.
        """
        self._interface.data.update(qos_limit=-1, qos_mode="statistics_only", qos_policy_ref=None)
    ...
    def full_qos(self, qos_limit, qos_policy):
        ...
        """
        self._interface.data.update(
            qos_limit=qos_limit, qos_mode="full_qos", qos_policy_ref=qos_policy.href
        )
...
class Interface(SubElement):
    ...
    def all_interfaces(self):
        ...
        """
        return AllInterfaces([self.vlan_interface, self.interfaces, self.port_group_interface])
    ...
    def cluster_virtual_interface(self):
        return [
            ...
            if getattr(interface, "typeof", None) == "cluster_virtual_interface"
        ]
    ...
    def get_boolean(self, name):
        ...
        for interface in self.all_interfaces:
            if isinstance(interface, VlanInterface):
                if any(vlan for vlan in interface.interfaces if getattr(vlan, name)):
                    return True
            else:
    ...
    def change_interface_id(self, interface_id):
        ...
        for interface in self.all_interfaces:
            ...
            if isinstance(interface, VlanInterface):
                interface.interface_id = "{}.{}".format(if1, interface.interface_id.split(".")[-1])
...
class TunnelInterface(Interface):
    ...
    def __init__(self, engine=None, meta=None, **interface):
        if not meta:
            meta = {key: interface.pop(key) for key in ("href", "type", "name") if key in interface}
    ...
    def _add_interface(self, interface_id, **kw):
        ...
        base_interface.update(interface_id=str(interface_id), interfaces=[])
        ...
        if interfaces:
            for interface in interfaces:
                if (
                    interface.get("cluster_virtual", None) or len(interface.get("nodes", [])) > 1
                ):  # Cluster
                    ...
                    for node in interface.get("nodes", []):
                        ...
                        base_interface.setdefault("interfaces", []).append({sni.typeof: sni.data})
        ...
        for interface in port_group_interfaces:
            ...
            for _interface in interfaces:
                ...
                for node in _interface.get("nodes", []):
                    _node = if_mgt.copy()
                    # Override management if set within interface
                    _node.update(node)
                    sni = SingleNodeInterface.create(interface_id, **_node)
                    sub_interface.setdefault("interfaces", []).append({sni.typeof: sni.data})
    ...
    def update_interface(self, other_interface, ignore_mgmt=True):
        ...
        for other in other_interface.port_group_interface:
            try:
                ...
                if (
                    len(this.interfaces) != len(other.interfaces)
                    or len(this.interfaces) > 1
                    or len(other.interfaces) > 1
                ):
                    ...
                    for interface in this.interfaces:
                        for _other in other.interfaces:
                            ...
                            # if getattr(_other, 'dynamic', None)
                            #   and getattr(interface, 'address', None):
                            #    interface.pop('address')
                            #    interface.pop('network_value')
                            # if getattr(_other, 'address', None)
                            #   and getattr(interface, 'dynamic', None):
...
class PhysicalInterface(Interface):
    ...
    def ndi_interfaces(self):
        ...
        """
        return [
            {"address": interface.address, "nicid": interface.nicid}
            for interface in self.interfaces
    ...
    def change_vlan_id(self, original, new):
        ...
        interface. When changing a single or cluster engine vlan, you
    ...
    def static_arp_entry(self, ipaddress, macaddress, arp_type="static", netmask=32):
        ...
        self.data["arp_entry"].append(
            {"ipaddress": ipaddress, "macaddress": macaddress, "netmask": netmask, "type": arp_type}
        )
    ...
    def virtual_mapping(self):
        ...
        if "virtual_mapping" in self.data:
            ...
            "virtual_resource_settings" in self.data
            and len(self.data["virtual_resource_settings"]) > 0
            and "virtual_mapping" in self.data["virtual_resource_settings"][0]
        ):
            return int(self.data["virtual_resource_settings"][0]["virtual_mapping"])
    ...
    def virtual_resource_name(self):
        ...
        if (
            ...
            return self.data["virtual_resource_settings"][0]["virtual_resource_name"]
    ...
    def _add_interface(self, interface_id, **kw):
        ...
        for interface in interfaces:
            ...
            if interface.get("vlan_id") and clz is not CaptureInterface:
                ...
                # Only used for L2/IPS engines
                second_vlan_id = interface.get("second_vlan_id")
                vlan_interface = {
                ...
                _inline_intf = clz.create(
                    interface_id="{}.{}".format(interface_id, vlan_id),
                    second_interface_id="{}.{}".format(
                        kw.get("second_interface_id"), second_vlan_id if second_vlan_id else vlan_id
                    ),
                    logical_interface_ref=logical_intf_helper(
                ...
                self.data.setdefault("vlanInterfaces", []).append(vlan_interface)
...
class Layer3PhysicalInterface(PhysicalInterface):
    ...
    :param str interface: specifies the type of interface to create. The interface
        ...
        single engine. For single engine, specify `single_node_interface`
    ...
    def _add_interface(self, interface_id, interface="node_interface", **kw):
        ...
        for interface in interfaces:
            ...
            for node in interface.pop("nodes", []):
                ...
                _node.update(node)
                ndi = clz.create(interface_id=_interface_id, **_node)
            ...
            if vlan_id:
                ...
                elif interface.get("virtual_resource_settings", None) is not None:
                    vlan_interface.update(
                        virtual_resource_settings=interface.pop("virtual_resource_settings")
                    )
                ...
                self.data.setdefault("interfaces", []).extend(_interface)
...
class ClusterPhysicalInterface(PhysicalInterface):
    ...
    :param dict nodes: nodes dict should contain keys `address`, `network_value` and
        `nodeid`. Overridden sub interface settings can also be set here
    :param str,href zone_ref: zone reference, name or zone.
    ...
    def _add_interface(self, interface_id, mgt=None, **kw):
        ...
        if "macaddress" in _kw:
            self.data.update(macaddress=_kw.pop("macaddress"))
        ...
        for interface in interfaces:
            ...
            for node in interface.pop("nodes", []):
                ...
                # also be used to override management settings
                _node.update(node)
                ndi = NodeInterface.create(interface_id=_interface_id, **_node)
                _interface.append({ndi.typeof: ndi.data})
    ...
    def __str__(self):
        return "{}(name={})".format(
            self.__class__.__name__, self.name if self.name else "Interface %s" % self.interface_id
        )
...
class PortGroupInterface(object):
    ...
    def ndi_interfaces(self):
        ...
        """
        return [
            {"address": interface.address, "nicid": getattr(self, "interface_id")}
    ...
    def delete(self):
        ...
        if self in self._parent.vlan_interface:
            self._parent.data["vlanInterfaces"] = [
                v for v in self._parent.vlan_interface if v != self
            ]
            self.update()
...
class VlanCollection(BaseIterable):
    ...
    def __init__(self, interface):
        ...
        data = [
            type(
                name,
                (VlanInterface, interface.__class__),
    ...
    def get_vlan(self, *args, **kwargs):
        ...
        for vlan in self:
            if getattr(vlan, key, None) == value:
                return vlan
        raise InterfaceNotFound("VLAN ID {} was not found on this engine.".format(value))
    ...
    def get(self, *args, **kwargs):
        ...
        if raise_exc:
            raise InterfaceNotFound(
                "Port Group %s was not found on this engine." % intf
                if intf
                else "Port group does not exist on this engine"
            )
...
class InterfaceEditor(object):
                ...
                if "SWP_" in interface_id and "SWP_" in intf.interface_id:
                    ...
                    for interface in intf.interfaces:
                        if isinstance(interface, InlineInterface):
                            ...
                            if interface_id == interface.nicid or str(interface_id) in split_intf:
                                intf._engine = self.engine
                                return intf
    ...
    def set_unset(self, interface_id, attribute, address=None):
        ...
        if address is not None:
            ...
            if not target_network:
                raise InterfaceNotFound(
                    "Address specified: %s was not found on interface "
        ...
        for interface in self:
            ...
            for sub_interface in all_subs:
                # Skip top level VLAN and inline interfaces (they have no
                # addresses)
                if not isinstance(
    ...
    def set_auth_request(self, interface_id, address=None):
        ...
        if "cluster" in self.engine.type:
            ...
            if not any(
                isinstance(itf, ClusterVirtualInterface) for itf in interface.all_interfaces
            ):
                raise InterfaceNotFound(
                    "The interface specified: %s does not have "
        ...
        current_interface = self.get(self.engine.interface_options.auth_request)
